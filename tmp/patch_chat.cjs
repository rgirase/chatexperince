...
if (modified === content) {
    // try to match with regex to be safer
    const regex = /const handleClearChat = \(\) => \{[\s\S]*?localStorage\.setItem\(`chat_\$\{persona\.id\}`,\s*JSON\.stringify\(initialMessage\)\);\s*\}\s*\};/;
    if (regex.test(content)) {
        modified = content.replace(regex, newCode);
    }
}
