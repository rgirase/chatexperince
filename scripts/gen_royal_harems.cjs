...
const characterIds = ['royal_harem_western', 'royal_harem_indian'];
...
async function generateHarems() {
    console.log(`🚀 Starting Generation for Dual Royal Harems (Western & Indian)...`);
    ...
    for (const characterId of characterIds) {
        ...
        console.log(`\n👑 Generating assets for: ${charDef.name}`);
                ...
                const response = await fetch('http://127.0.0.1:8000/prompt', {
                    ...
                    body: JSON.stringify({
                        "prompt": {
                            ...
                            "7": { "class_type": "CLIPTextEncode", "inputs": { "text": "bad eyes, bad hands, low quality, blurry, deformed, cartoon, anime, 3d, rendering, low resolution", "clip": ["4", 1] } },
                            "8": { "class_type": "VAEDecode", "inputs": { "samples": ["3", 0], "vae": ["4", 2] } },
                            "9": { "class_type": "SaveImage", "inputs": { "filename_prefix": `royal_harem_`, "images": ["8", 0] } }
                        }
                    })
                });
    ...
    console.log(`\n⭐ All royal harem asset generation complete!`);
}
